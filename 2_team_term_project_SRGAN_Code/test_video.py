...
if __name__ == "__main__":
    ...
    fps = videoCapture.get(cv2.CAP_PROP_FPS) #비디오의 프레임 속도를 측정
    frame_numbers = videoCapture.get(cv2.CAP_PROP_FRAME_COUNT) #총 프레임의 수를 측정
    #SR을 한 비디오
    sr_video_size = (int(videoCapture.get(cv2.CAP_PROP_FRAME_WIDTH) * UPSCALE_FACTOR), #프레임 너비 * UPSCALE_FACTOR
                     int(videoCapture.get(cv2.CAP_PROP_FRAME_HEIGHT)) * UPSCALE_FACTOR) #프레임 높이 * UPSCALE_FACTOR
    ...
    output_sr_name = 'out_srf_' + str(UPSCALE_FACTOR) + '_' + VIDEO_NAME.split('.')[0] + '.mp4' #SR을 한뒤 내보낼 영상
    output_compared_name = 'compare_srf_' + str(UPSCALE_FACTOR) + '_' + VIDEO_NAME.split('.')[0] + '.mp4'
    #영상 저장하기, codec, fps, video_size
    ...
    compared_video_writer = cv2.VideoWriter(output_compared_name, cv2.VideoWriter_fourcc('M', 'P', 'E', 'G'), fps,
                                            compared_video_size)
    # read frame
    success, frame = videoCapture.read() #비디오의 한 프레임씩 읽음. 제대로 읽으면 success값이 True
    test_bar = tqdm(range(int(frame_numbers)), desc='[processing video and saving result videos]')
    for index in test_bar:
        if success: #비디오를 제대로 읽으면
            image = Variable(ToTensor()(frame), volatile=True).unsqueeze(0) #frame 이미지를 텐서로 바꿔주어 image에 저장.
            if torch.cuda.is_available():
                image = image.cuda()

            out = model(image) #SR을 한 이미지를 out에 저장.
            out = out.cpu()
            out_img = out.data[0].numpy()
            out_img *= 255.0 #out_img의 값은 0~1사이의 값이므로 255를 곱해 0~255로 바꿔줌.
            out_img = (np.uint8(out_img)).transpose((1, 2, 0))
            # save sr video
            sr_video_writer.write(out_img) #이미지를 저장.
